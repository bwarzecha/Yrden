...
struct IteratorCancellationTests {
    ...
    func cancelDuringModelCallThrowsError() async throws {
        ...
        let agent = try Agent<String>(model: slowModel)
    ...
    func newIterationAfterCancelIndependent() async throws {
        ...
        let task1 = Task {
            for try await _ in agent.iter("Task 1") { }
        }
        try await Task.sleep(for: .milliseconds(5))
        task1.cancel()
        ...
        var output: String?
        for try await node in agent.iter("Fresh start") {
            if case .finished(let ctx) = node {
                output = ctx.output
            }
        }
    ...
    func concurrentIterationsCancelIndependently() async throws {
        ...
        let task2 = Task {
            var output: String?
            for try await node in agent.iter("Task 2") {
                if case .finished(let ctx) = node {
                    output = ctx.output
                }
            }
    ...
    func cancellationPropagatesThroughNestedAgentTool() async throws {
        ...
        let innerAgent = try Agent<String>(model: innerModel)
        ...
        let nestedTool = FakeTool<ConfigurableToolArgs, String>(
            ...
            onCall: { _ in
                do {
                    for try await node in innerAgent.iter("Inner") {
                        if case .finished(let ctx) = node {
        ...
        let outerAgent = try Agent<String>(
            model: outerModel,
            tools: [nestedTool]
        )

        let task = Task {
            for try await _ in outerAgent.iter("Use nested") { }
        }
